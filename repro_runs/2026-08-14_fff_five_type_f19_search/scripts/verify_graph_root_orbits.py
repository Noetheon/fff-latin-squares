...
def main() -> None:
    ...
    if not all(dimension_checks.values()):
        raise SystemExit("invalid f19 dimension record")
    ...
    if root != expected_root:
        raise SystemExit("graph does not contain the canonical f19 root")
    ...
    expected_orbit_size_counts = {12: 6, 24: 2, 48: 80}
    if orbit_size_counts != expected_orbit_size_counts:
        raise SystemExit("unexpected f19 orbit-size distribution")
    ...
    args.summary.write_text("\n".join([
        "Five-type f19 graph root-orbit audit", "",
