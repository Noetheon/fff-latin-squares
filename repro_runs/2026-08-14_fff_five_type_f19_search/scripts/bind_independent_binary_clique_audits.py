...
F19_GRAPH_SHA = "de95738a09826c3f37f72a4c090acdb632dfdee7d6c7521d567e8899ee024aa2"
...
def main() -> None:
    ...
    parser.add_argument("--f19-graph", type=Path, required=True)
    parser.add_argument("--f19-result", type=Path, required=True)
    ...
    f19 = json.loads(args.f19_result.read_text())
    ...
    checks = {
        "f19_graph_hash": sha256(args.f19_graph) == F19_GRAPH_SHA,
        "f18_graph_hash": sha256(args.f18_graph) == F18_GRAPH_SHA,
        "f19_result_valid": f19.get("valid") is True and zero_errors(f19),
        "f19_complete_counts": f19.get("initial_tasks") == 3960
        and f19.get("clique_count") == 1920
        and f19.get("pattern_counts") == {"FFT": 960, "FTF": 960}
        and f19.get("exact_palette_count") == 0
        and f19.get("fff_count") == 0,
    ...
    result = {
        "schema_version": "f19-independent-binary-clique-audit-binding-v1",
        "files": {
            "f19_graph": artifact(args.f19_graph),
            "f19_result": artifact(args.f19_result),
        ...
        "commands": {
            ...
            "f19_run": [str(args.binary), str(args.f19_graph),
                "2+2+2+2+2", "4+2+2+2", "6+2+2", "6+4", "8+2", "8"],
        },
        "checks": checks,
    ...
    args.summary.write_text("\n".join([
        ...
        f"f19 cliques/patterns: {f19.get('clique_count')}/{f19.get('pattern_counts')}",
        f"f19 exact-palette/FFF: {f19.get('exact_palette_count')}/{f19.get('fff_count')}",
