...
F06_GRAPH_SHA = "8fe34d2fdd3bc8e9eec6fb8cab01ba316ef54c4750413c74d4bf7ce35fb342bb"
F06_CLIQUE_COUNT = 2880
F06_PATTERN_COUNTS = {"FFT": 864, "FTF": 864, "FTT": 1152}
F06_EXACT_PALETTE_COUNT = 0
...
def main() -> None:
    ...
    parser.add_argument("--f06-graph", type=Path, required=True)
    parser.add_argument("--f06-result", type=Path, required=True)
    ...
    f06 = json.loads(args.f06_result.read_text())
    ...
    checks = {
        "f06_graph_hash": sha256(args.f06_graph) == F06_GRAPH_SHA,
        "f18_graph_hash": sha256(args.f18_graph) == F18_GRAPH_SHA,
        "f06_result_valid": f06.get("valid") is True and zero_errors(f06),
        "f06_complete_counts": f06.get("initial_tasks") == 13116
        and f06.get("clique_count") == F06_CLIQUE_COUNT
        and f06.get("pattern_counts") == F06_PATTERN_COUNTS
        and f06.get("exact_palette_count") == F06_EXACT_PALETTE_COUNT
        and f06.get("fff_count") == 0,
    ...
    result = {
        "schema_version": "f06-independent-binary-clique-audit-binding-v1",
        "files": {
            "f06_graph": artifact(args.f06_graph),
            "f06_result": artifact(args.f06_result),
        ...
        "commands": {
            ...
            "f06_run": [str(args.binary), str(args.f06_graph),
                "10", "2+2+2+2+2", "4+2+2+2", "6+4", "8+2", "8"],
        },
        "checks": checks,
    ...
    args.summary.write_text("\n".join([
        ...
        f"f06 cliques/patterns: {f06.get('clique_count')}/{f06.get('pattern_counts')}",
        f"f06 exact-palette/FFF: {f06.get('exact_palette_count')}/{f06.get('fff_count')}",
