...
def main() -> None:
    ...
    if not all(dimension_checks.values()):
        raise SystemExit("invalid f06 dimension record")
    ...
    if root != expected_root:
        raise SystemExit("graph does not contain the canonical f06 root")
    ...
    expected_orbit_size_counts = {12: 3, 24: 1, 48: 272}
    if orbit_size_counts != expected_orbit_size_counts:
        raise SystemExit("unexpected f06 orbit-size distribution")
    ...
    args.summary.write_text("\n".join([
        "Five-type f06 graph root-orbit audit", "",
