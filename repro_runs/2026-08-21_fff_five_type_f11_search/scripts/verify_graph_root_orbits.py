...
def main() -> None:
    ...
    if not all(dimension_checks.values()):
        raise SystemExit("invalid f11 dimension record")
    ...
    if root != expected_root:
        raise SystemExit("graph does not contain the canonical f11 root")
    ...
    expected_orbit_size_counts = {12: 5, 24: 2, 48: 254}
    if orbit_size_counts != expected_orbit_size_counts:
        raise SystemExit("unexpected f11 orbit-size distribution")
    ...
    args.summary.write_text("\n".join([
        "Five-type f11 graph root-orbit audit", "",
