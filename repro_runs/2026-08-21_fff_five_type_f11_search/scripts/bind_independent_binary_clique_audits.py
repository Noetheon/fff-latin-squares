...
F11_GRAPH_SHA = "d5b53aa9ddaf6ccdde118cb079020d431ae10abc39916d1e864a074f381e1f54"
F11_CLIQUE_COUNT = 384
F11_PATTERN_COUNTS = {"FTT": 384}
F11_EXACT_PALETTE_COUNT = 384
...
def main() -> None:
    ...
    parser.add_argument("--f11-graph", type=Path, required=True)
    parser.add_argument("--f11-result", type=Path, required=True)
    ...
    f11 = json.loads(args.f11_result.read_text())
    ...
    checks = {
        "f11_graph_hash": sha256(args.f11_graph) == F11_GRAPH_SHA,
        "f18_graph_hash": sha256(args.f18_graph) == F18_GRAPH_SHA,
        "f11_result_valid": f11.get("valid") is True and zero_errors(f11),
        "f11_complete_counts": f11.get("initial_tasks") == 12300
        and f11.get("clique_count") == F11_CLIQUE_COUNT
        and f11.get("pattern_counts") == F11_PATTERN_COUNTS
        and f11.get("exact_palette_count") == F11_EXACT_PALETTE_COUNT
        and f11.get("fff_count") == 0,
    ...
    result = {
        "schema_version": "f11-independent-binary-clique-audit-binding-v1",
        "files": {
            "f11_graph": artifact(args.f11_graph),
            "f11_result": artifact(args.f11_result),
        ...
        "commands": {
            ...
            "f11_run": [str(args.binary), str(args.f11_graph),
                "10", "4+2+2+2", "4+4+2", "6+2+2", "6+4", "8"],
        },
        "checks": checks,
    ...
    args.summary.write_text("\n".join([
        ...
        f"f11 cliques/patterns: {f11.get('clique_count')}/{f11.get('pattern_counts')}",
        f"f11 exact-palette/FFF: {f11.get('exact_palette_count')}/{f11.get('fff_count')}",
