...
def main() -> None:
    ...
    if not all(dimension_checks.values()):
        raise SystemExit("invalid f04 dimension record")
    ...
    if root != expected_root:
        raise SystemExit("graph does not contain the canonical f04 root")
    ...
    expected_orbit_size_counts = {12: 6, 24: 2, 48: 180}
    if orbit_size_counts != expected_orbit_size_counts:
        raise SystemExit("unexpected f04 orbit-size distribution")
    ...
    args.summary.write_text("\n".join([
        "Five-type f04 graph root-orbit audit", "",
