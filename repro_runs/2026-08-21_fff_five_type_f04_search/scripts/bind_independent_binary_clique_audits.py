...
F04_GRAPH_SHA = "e3c41c65e3a3dcb7a14d2870ede2173b2ab39cfa57924f727a867287b70ed953"
F04_CLIQUE_COUNT = 0
F04_PATTERN_COUNTS = {}
F04_EXACT_PALETTE_COUNT = 0
...
def main() -> None:
    ...
    parser.add_argument("--f04-graph", type=Path, required=True)
    parser.add_argument("--f04-result", type=Path, required=True)
    ...
    f04 = json.loads(args.f04_result.read_text())
    ...
    checks = {
        "f04_graph_hash": sha256(args.f04_graph) == F04_GRAPH_SHA,
        "f18_graph_hash": sha256(args.f18_graph) == F18_GRAPH_SHA,
        "f04_result_valid": f04.get("valid") is True and zero_errors(f04),
        "f04_complete_counts": f04.get("initial_tasks") == 8760
        and f04.get("clique_count") == F04_CLIQUE_COUNT
        and f04.get("pattern_counts") == F04_PATTERN_COUNTS
        and f04.get("exact_palette_count") == F04_EXACT_PALETTE_COUNT
        and f04.get("fff_count") == 0,
    ...
    result = {
        "schema_version": "f04-independent-binary-clique-audit-binding-v1",
        "files": {
            "f04_graph": artifact(args.f04_graph),
            "f04_result": artifact(args.f04_result),
        ...
        "commands": {
            ...
            "f04_run": [str(args.binary), str(args.f04_graph),
                "10", "2+2+2+2+2", "4+2+2+2", "6+2+2", "6+4", "8"],
        },
        "checks": checks,
    ...
    args.summary.write_text("\n".join([
        ...
        f"f04 cliques/patterns: {f04.get('clique_count')}/{f04.get('pattern_counts')}",
        f"f04 exact-palette/FFF: {f04.get('exact_palette_count')}/{f04.get('fff_count')}",
