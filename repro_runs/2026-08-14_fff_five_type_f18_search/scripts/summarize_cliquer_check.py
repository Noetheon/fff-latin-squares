...
def main() -> None:
    ...
    found_clique = bool(re.search(r"size=8,\s+weight=8:", text))
    no_clique = "No such clique found." in text and maxima and max(maxima) <= 7
    result = {
        "schema_version": "f18-independent-cliquer-check-v1",
        ...
        "result": (
            "clique_of_size_8_found" if found_clique else
            "no_clique_of_size_8" if no_clique else "unrecognized"
        ),
        "files": {
        ...
        "valid": bool(found_clique or no_clique),
    }
    args.output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
    args.summary.write_text("\n".join([
        "Independent Cliquer 1.21 f18 check", "",
    ...
    if not result["valid"]:
        raise SystemExit("Cliquer result not recognized")
