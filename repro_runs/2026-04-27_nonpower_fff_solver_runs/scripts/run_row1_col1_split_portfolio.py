#!/usr/bin/env python3
"""Generate and solve selected row1+col1 split CNFs for n=10."""
...
def select_cases(cases: list[dict[str, Any]], args: argparse.Namespace) -> list[dict[str, Any]]:
    selected = cases
    if args.row1_case_id:
        selected = [case for case in selected if case["row1_case_id"] == args.row1_case_id]
    if args.case_ids:
...
def generate_case_cnf(case: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    suffix = safe_case_suffix(case)
    prefix = f"n10_{view_label(args.views)}_row1col1case_{suffix}"
    paths = {
    ...
    if run["returncode"] != 0:
        raise RuntimeError(f"CNF generation failed for {case['case_id']}")
    meta = read_json(paths["metadata"])
    return {
        "case_id": case["case_id"],
        "row1_case_id": case["row1_case_id"],
        "col1_case_id": case["col1_case_id"],
        "row1_partition": case["row1_partition"],
        "row1_zero_cycle_length": case["row1_zero_cycle_length"],
        "row1_permutation": case["row1_permutation"],
        "col1_partition": case["col1_partition"],
        "col1_zero_cycle_length": case["col1_zero_cycle_length"],
        "col1_permutation": case["col1_permutation"],
        "residual_group_size": case["residual_group_size"],
...
def solve_case(generated: dict[str, Any], solver: str, timeout: float, args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "col1_case_id": generated["col1_case_id"],
        "row1_permutation": generated["row1_permutation"],
        "col1_permutation": generated["col1_permutation"],
...
def make_summary(payload: dict[str, Any]) -> str:
    lines = [
        f"n=10 {payload['view_label']} row1+col1 split portfolio",
        "",
        f"full_case_count: {payload['full_case_count']}",
        f"selected_case_count: {payload['selected_case_count']}",
        ...
        f"complete_portfolio: {payload['complete_portfolio']}",
        "",
        "Result counts:",
    ]
    counts: dict[str, int] = {}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--views", default="row,col", choices=["row,col", "row,col,sym"])
    ...
    parser.add_argument("--row1-case-id")
...
def main() -> int:
    ...
    generated_cases = [generate_case_cnf(case, args) for case in selected_cases]
    ...
    complete_portfolio = len(selected_cases) == len(all_cases) and not args.generate_only
    views = args.views.split(",")
    view = view_label(args.views)
    claim_note = (
        "C38 remains open and C40 is not created unless the complete row1+col1 split is decided "
        "or a full row,col,sym SAT model validates as FFF. A selected-subset portfolio is diagnostic only."
    )
    payload = {
        ...
        "selection": {
            "row1_case_id": args.row1_case_id,
