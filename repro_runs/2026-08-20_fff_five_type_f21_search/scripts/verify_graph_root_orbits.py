...
def main() -> None:
    ...
    if not all(dimension_checks.values()):
        raise SystemExit("invalid f21 dimension record")
    ...
    if root != expected_root:
        raise SystemExit("graph does not contain the canonical f21 root")
    ...
    expected_orbit_size_counts = {12: 5, 24: 2, 48: 155}
    if orbit_size_counts != expected_orbit_size_counts:
        raise SystemExit("unexpected f21 orbit-size distribution")
    ...
    args.summary.write_text("\n".join([
        "Five-type f21 graph root-orbit audit", "",
