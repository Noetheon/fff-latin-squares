...
GRAPH_SHA256 = "35fac04cc7566d4fe6aa0be5a96c0e06a6c92804936de60311324a76372f7a4e"
...
def main() -> None:
    ...
    structural = load(results / "f21_structural_graph_audit.json")
    semantic = load(results / "f21_complete_graph_audit.json")
    binding = load(results / "f21_graph_audit_binding.json")
    source_audit = load(results / "f21_source_generalization_audit.json")
    orbit = load(results / "f21_graph_root_orbit_audit.json")
    sliced_plan = load(results / "f21_task_plan_sliced.json")
    monolithic_plan = load(results / "f21_task_plan_monolithic.json")
    full_plan = load(results / "f21_task_plan_full_root_bucket.json")
    sliced = load(results / "f21_final_sliced_merge.json")
    monolithic = load(results / "f21_final_monolithic_merge.json")
    full = load(results / "f21_full_root_bucket_merge.json")
    equivalence = load(results / "f21_sliced_monolithic_equivalence.json")
    independent = load(results / "f21_independent_binary_clique_binding.json")
    graph = Path(binding["graph_path"])
    task_file = args.run / "data" / "f21_task_ordinals.txt"

    expected_orbit_sizes = {"12": 5, "24": 2, "48": 155}
    ...
    checks = {
        ...
        and structural.get("vertices") == 74832
        and structural.get("words") == 1170
        and structural.get("actual_size_bytes") == 701175866,
        ...
        and semantic.get("stored_vertices") == 74832
        and semantic.get("independently_enumerated_vertices") == 74832
        and semantic.get("pairs_checked") == 2799876696
        and semantic.get("edges") == 259300944,
        ...
        == sha256(results / "f21_structural_graph_audit.json")
        and binding.get("semantic_audit_sha256")
        == sha256(results / "f21_complete_graph_audit.json"),
        ...
        "orbit_cover_exact": orbit.get("full_initial_task_count") == 7548
        and orbit.get("orbit_count") == 162
        and orbit.get("orbit_size_counts") == expected_orbit_sizes
        and orbit.get("covered_task_count") == 7548
        ...
        and len(task_file.read_text().splitlines()) == 162,
        ...
        "reduced_sliced_complete_negative": complete_negative(
            sliced, 162, 32721617, 19030045
        ),
        "reduced_monolithic_complete_negative": complete_negative(
            monolithic, 162, 32721617, 19030045
        ),
        ...
        and full_plan["tasking"]["total_initial_tasks"] == 7548,
        "full_root_bucket_complete_negative": complete_negative(
            full, 7548, 1502429376, 871086822
        ),
        ...
        and independent["files"]["f21_graph"]["sha256"] == GRAPH_SHA256
        and independent["files"]["f21_result"]["sha256"]
        == sha256(results / "f21_binary_clique_table_audit.json")
        and independent["files"]["source"]["sha256"]
        == sha256(args.run / "scripts" / "enumerate_binary_graph_clique_tables.cpp"),
    }
    result = {
        "schema_version": "five-type-f21-run-validation-v1",
        "checks": checks,
        "all_checks_passed": all(checks.values()),
        "result": {
            "case_id": "f21",
            "palette": ["4+2+2+2", "4+4+2", "6+2+2", "6+4", "8+2"],
            "graph_sha256": GRAPH_SHA256,
            "vertices": 74832,
            "edges": 259300944,
            "orbits": 162,
            "full_tasks": 7548,
            "reduced_nodes": 32721617,
            "full_root_nodes": 1502429376,
            "fff_found": False,
    ...
    args.summary.write_text("\n".join([
        "Five-type f21 run validation", "",
        f"checks passed: {sum(checks.values())}/{len(checks)}",
        "graph vertices/edges: 74832/259300944",
        "full tasks/orbits: 7548/162",
        "reduced/full-root nodes: 32721617/1502429376",
    ...
    if not result["all_checks_passed"]:
        ...
        raise SystemExit(f"f21 validation failed: {failed}")
