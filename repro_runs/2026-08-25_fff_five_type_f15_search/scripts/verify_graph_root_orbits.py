...
EXPECTED_PALETTE = ["10", "4+4+2", "6+2+2", "6+4", "8+2"]
EXPECTED_ROOT = (1, 2, 3, 0, 5, 6, 7, 4, 9, 8)
...
def main() -> None:
    ...
    dimension_checks = {
        ...
        "case": dimension.get("case_id") == "f15",
        "palette": dimension.get("palette") == EXPECTED_PALETTE,
        "root_type": dimension.get("root_type") == "4+4+2",
        "expected_counts": dimension.get("level1_candidates") == 145152
        and dimension.get("full_initial_tasks") == 14952
        and dimension.get("orbit_count") == 1872,
    }
    if not all(dimension_checks.values()):
        raise SystemExit("invalid f15 dimension record")
    ...
    if root != EXPECTED_ROOT:
        raise SystemExit("graph does not contain the canonical f15 root")
    ...
    if orbit_size_counts != EXPECTED_ORBIT_SIZES:
        raise SystemExit("unexpected f15 orbit-size distribution")

    expected_task_text = "".join(f"{ordinal}\n" for ordinal in representatives)
    if args.task_ordinals.is_file():
        if args.task_ordinals.read_text() != expected_task_text:
            raise SystemExit("frozen task-ordinal input differs from reconstructed orbits")
    else:
        args.task_ordinals.write_text(expected_task_text)
    ...
    args.summary.write_text("\n".join([
        "Five-type f15 graph root-orbit audit", "",
