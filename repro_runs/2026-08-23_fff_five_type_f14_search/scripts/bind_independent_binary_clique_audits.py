...
F14_GRAPH_SHA = "18bae6616c5ff5ae053761d4baae826b506510cab6a1098e361e472cec25bf88"
...
def main() -> None:
    ...
    parser.add_argument("--f14-graph", type=Path, required=True)
    parser.add_argument("--f14-result", type=Path, required=True)
    ...
    f14 = json.loads(args.f14_result.read_text())
    f18 = json.loads(args.f18_result.read_text())
    f18_reference = json.loads(args.f18_reference_audit.read_text())
    f14_patterns = f14.get("pattern_counts", {})
    checks = {
        "f14_graph_hash": sha256(args.f14_graph) == F14_GRAPH_SHA,
        "f18_graph_hash": sha256(args.f18_graph) == F18_GRAPH_SHA,
        "f14_result_valid": f14.get("valid") is True and zero_errors(f14),
        "f14_complete_task_cover": f14.get("initial_tasks") == 16020,
        "f14_pattern_partition": sum(f14_patterns.values()) == f14.get("clique_count")
        and f14_patterns.get("FFF", 0) == f14.get("fff_count"),
        "f14_no_fff": f14.get("fff_count") == 0,
        "f14_exact_palette_count_bounded": 0 <= f14.get("exact_palette_count", -1)
        <= f14.get("clique_count", -2),
    ...
    result = {
        "schema_version": "f14-independent-binary-clique-audit-binding-v1",
        "files": {
            "f14_graph": artifact(args.f14_graph),
            "f14_result": artifact(args.f14_result),
        ...
        "commands": {
            ...
            "f14_run": [str(args.binary), str(args.f14_graph),
                "10", "4+2+2+2", "6+2+2", "6+4", "8+2", "8"],
        },
        "checks": checks,
    ...
    args.summary.write_text("\n".join([
        ...
        f"f14 cliques/patterns: {f14.get('clique_count')}/{f14_patterns}",
        f"f14 exact-palette/FFF: {f14.get('exact_palette_count')}/{f14.get('fff_count')}",
