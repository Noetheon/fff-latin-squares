...
GRAPH_SHA256 = "18bae6616c5ff5ae053761d4baae826b506510cab6a1098e361e472cec25bf88"
FULL_ROOT_NODES = 44027826160
FULL_ROOT_PRUNES = 29837674701
TRANSVERSAL_CLIQUE_COUNT = 21312
EXACT_PALETTE_CLIQUE_COUNT = 18432
...
def main() -> None:
    ...
    structural = load(results / "f14_structural_graph_audit.json")
    semantic = load(results / "f14_complete_graph_audit.json")
    binding = load(results / "f14_graph_audit_binding.json")
    source_audit = load(results / "f14_source_generalization_audit.json")
    orbit = load(results / "f14_graph_root_orbit_audit.json")
    sliced_plan = load(results / "f14_task_plan_sliced.json")
    monolithic_plan = load(results / "f14_task_plan_monolithic.json")
    full_plan = load(results / "f14_task_plan_full_root_bucket.json")
    sliced = load(results / "f14_final_sliced_merge.json")
    monolithic = load(results / "f14_final_monolithic_merge.json")
    full = load(results / "f14_full_root_bucket_merge.json")
    equivalence = load(results / "f14_sliced_monolithic_equivalence.json")
    independent = load(results / "f14_independent_binary_clique_binding.json")
    graph = Path(binding["graph_path"])
    task_file = args.run / "data" / "f14_task_ordinals.txt"

    expected_orbit_sizes = {"12": 5, "24": 1, "48": 332}
    ...
    checks = {
        ...
        and structural.get("vertices") == 150240
        and structural.get("words") == 2348
        and structural.get("actual_size_bytes") == 2823610586,
        ...
        and semantic.get("stored_vertices") == 150240
        and semantic.get("independently_enumerated_vertices") == 150240
        and semantic.get("pairs_checked") == 11285953680
        and semantic.get("edges") == 1174874016,
        ...
        == sha256(results / "f14_structural_graph_audit.json")
        and binding.get("semantic_audit_sha256")
        == sha256(results / "f14_complete_graph_audit.json"),
        ...
        "orbit_cover_exact": orbit.get("full_initial_task_count") == 16020
        and orbit.get("orbit_count") == 338
        and orbit.get("orbit_size_counts") == expected_orbit_sizes
        and orbit.get("covered_task_count") == 16020
        ...
        and len(task_file.read_text().splitlines()) == 338,
        ...
        "reduced_sliced_complete_negative": complete_negative(
            sliced, 338, 947916796, 643352771
        ),
        "reduced_monolithic_complete_negative": complete_negative(
            monolithic, 338, 947916796, 643352771
        ),
        ...
        and full_plan["tasking"]["total_initial_tasks"] == 16020,
        "full_root_bucket_complete_negative": complete_negative(
            full, 16020, FULL_ROOT_NODES, FULL_ROOT_PRUNES
        ),
        ...
        and independent["files"]["f14_graph"]["sha256"] == GRAPH_SHA256
        and independent["files"]["f14_result"]["sha256"]
        == sha256(results / "f14_binary_clique_table_audit.json")
        and independent["files"]["source"]["sha256"]
        == sha256(args.run / "scripts" / "enumerate_binary_graph_clique_tables.cpp"),
    }
    result = {
        "schema_version": "five-type-f14-run-validation-v1",
        "checks": checks,
        "all_checks_passed": all(checks.values()),
        "result": {
            "case_id": "f14",
            "palette": ["10", "4+2+2+2", "6+2+2", "6+4", "8+2"],
            "graph_sha256": GRAPH_SHA256,
            "vertices": 150240,
            "edges": 1174874016,
            "orbits": 338,
            "full_tasks": 16020,
            "reduced_nodes": 947916796,
    ...
    args.summary.write_text("\n".join([
        "Five-type f14 run validation", "",
        f"checks passed: {sum(checks.values())}/{len(checks)}",
        "graph vertices/edges: 150240/1174874016",
        "full tasks/orbits: 16020/338",
    ...
    if not result["all_checks_passed"]:
        ...
        raise SystemExit(f"f14 validation failed: {failed}")
