...
EXPECTED_PALETTE = ["10", "4+2+2+2", "6+2+2", "6+4", "8+2"]
...
def main() -> None:
    ...
    dimension_checks = {
        ...
        "case": dimension.get("case_id") == "f14",
        "palette": dimension.get("palette") == EXPECTED_PALETTE,
        "root_type": dimension.get("root_type") == "4+2+2+2",
        "expected_counts": dimension.get("level1_candidates") == 150240
        and dimension.get("full_initial_tasks") == 16020
        and dimension.get("orbit_count") == 338,
    }
    if not all(dimension_checks.values()):
        raise SystemExit("invalid f14 dimension record")
    ...
    if root != EXPECTED_ROOT:
        raise SystemExit("graph does not contain the canonical f14 root")
    ...
    if orbit_size_counts != EXPECTED_ORBIT_SIZES:
        raise SystemExit("unexpected f14 orbit-size distribution")
    ...
    args.summary.write_text("\n".join([
        "Five-type f14 graph root-orbit audit", "",
