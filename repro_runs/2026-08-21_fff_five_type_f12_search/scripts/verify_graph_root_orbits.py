...
def main() -> None:
    ...
    if not all(dimension_checks.values()):
        raise SystemExit("invalid f12 dimension record")
    ...
    if root != expected_root:
        raise SystemExit("graph does not contain the canonical f12 root")
    ...
    expected_orbit_size_counts = {12: 2, 24: 1, 48: 293}
    if orbit_size_counts != expected_orbit_size_counts:
        raise SystemExit("unexpected f12 orbit-size distribution")
    ...
    args.summary.write_text("\n".join([
        "Five-type f12 graph root-orbit audit", "",
