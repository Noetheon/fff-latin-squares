...
F12_GRAPH_SHA = "09ac783d78c5762c283e61e19e2ab4c21a12fba9f5b16588d3eba9b9e2579ef7"
F12_CLIQUE_COUNT = 1920
F12_PATTERN_COUNTS = {"FFT": 384, "FTF": 384, "FTT": 1152}
F12_EXACT_PALETTE_COUNT = 1152
...
def main() -> None:
    ...
    parser.add_argument("--f12-graph", type=Path, required=True)
    parser.add_argument("--f12-result", type=Path, required=True)
    ...
    f12 = json.loads(args.f12_result.read_text())
    ...
    checks = {
        "f12_graph_hash": sha256(args.f12_graph) == F12_GRAPH_SHA,
        "f18_graph_hash": sha256(args.f18_graph) == F18_GRAPH_SHA,
        "f12_result_valid": f12.get("valid") is True and zero_errors(f12),
        "f12_complete_counts": f12.get("initial_tasks") == 14112
        and f12.get("clique_count") == F12_CLIQUE_COUNT
        and f12.get("pattern_counts") == F12_PATTERN_COUNTS
        and f12.get("exact_palette_count") == F12_EXACT_PALETTE_COUNT
        and f12.get("fff_count") == 0,
    ...
    result = {
        "schema_version": "f12-independent-binary-clique-audit-binding-v1",
        "files": {
            "f12_graph": artifact(args.f12_graph),
            "f12_result": artifact(args.f12_result),
        ...
        "commands": {
            ...
            "f12_run": [str(args.binary), str(args.f12_graph),
                "10", "4+2+2+2", "4+4+2", "6+2+2", "8+2", "8"],
        },
        "checks": checks,
    ...
    args.summary.write_text("\n".join([
        ...
        f"f12 cliques/patterns: {f12.get('clique_count')}/{f12.get('pattern_counts')}",
        f"f12 exact-palette/FFF: {f12.get('exact_palette_count')}/{f12.get('fff_count')}",
