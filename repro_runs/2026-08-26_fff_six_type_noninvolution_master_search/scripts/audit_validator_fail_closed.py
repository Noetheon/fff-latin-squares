...
def load_validator(path: Path):
    spec = importlib.util.spec_from_file_location("validate_noninvolution", path)
...
def main() -> None:
    ...
    tasks, nodes, prunes = 386, 123456789, 98765432
    baseline = {
    ...
    result = {
        "schema_version": "noninvolution-master-validator-fail-closed-v1",
        "checks": checks, "observations": observations,
        "all_checks_passed": all(checks.values()),
    }
    args.output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
    args.summary.write_text(
        "Noninvolution master validator fail-closed audit\n\n"
