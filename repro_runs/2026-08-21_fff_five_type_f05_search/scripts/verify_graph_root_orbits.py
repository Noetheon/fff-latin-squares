...
def main() -> None:
    ...
    if not all(dimension_checks.values()):
        raise SystemExit("invalid f05 dimension record")
    ...
    if root != expected_root:
        raise SystemExit("graph does not contain the canonical f05 root")
    ...
    expected_orbit_size_counts = {12: 3, 48: 247}
    if orbit_size_counts != expected_orbit_size_counts:
        raise SystemExit("unexpected f05 orbit-size distribution")
    ...
    args.summary.write_text("\n".join([
        "Five-type f05 graph root-orbit audit", "",
