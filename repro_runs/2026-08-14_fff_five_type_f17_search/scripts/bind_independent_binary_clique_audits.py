...
F17_GRAPH_SHA = "8958816838da8c47f29af21230c1bfb2bb78d2597f57d80fff48c85ab8f7bd78"
...
def main() -> None:
    ...
    parser.add_argument("--f17-graph", type=Path, required=True)
    parser.add_argument("--f17-result", type=Path, required=True)
    ...
    f17 = json.loads(args.f17_result.read_text())
    ...
    checks = {
        "f17_graph_hash": sha256(args.f17_graph) == F17_GRAPH_SHA,
        "f18_graph_hash": sha256(args.f18_graph) == F18_GRAPH_SHA,
        "f17_result_valid": f17.get("valid") is True and zero_errors(f17),
        "f17_no_eight_clique": f17.get("initial_tasks") == 3372
        and f17.get("clique_count") == 0,
    ...
    result = {
        "schema_version": "f17-independent-binary-clique-audit-binding-v1",
        "files": {
            "f17_graph": artifact(args.f17_graph),
            "f17_result": artifact(args.f17_result),
        ...
        "commands": {
            ...
            "f17_run": [str(args.binary), str(args.f17_graph),
                "2+2+2+2+2", "4+2+2+2", "4+4+2", "6+2+2", "8+2", "8"],
        },
        "checks": checks,
    ...
    args.summary.write_text("\n".join([
        ...
        f"f17 8-cliques: {f17.get('clique_count')}",
