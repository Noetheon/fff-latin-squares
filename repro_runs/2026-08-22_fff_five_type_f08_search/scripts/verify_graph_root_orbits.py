...
def main() -> None:
    ...
    if not all(dimension_checks.values()):
        raise SystemExit("invalid f08 dimension record")
    ...
    if root != expected_root:
        raise SystemExit("graph does not contain the canonical f08 root")
    ...
    expected_orbit_size_counts = {48: 300}
    if orbit_size_counts != expected_orbit_size_counts:
        raise SystemExit("unexpected f08 orbit-size distribution")
    ...
    args.summary.write_text("\n".join([
        "Five-type f08 graph root-orbit audit", "",
