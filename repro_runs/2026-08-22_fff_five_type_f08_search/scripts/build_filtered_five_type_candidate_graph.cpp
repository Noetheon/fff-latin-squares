...
static bool allowed(const Type &type, const std::array<Type, 5> &palette) {
  return std::find(palette.begin(), palette.end(), type) != palette.end();
}

static bool allowed_code(uint64_t code, const std::array<uint64_t, 5> &palette) {
...
int main(int argc, char **argv) {
  if (argc != 11 && argc != 12) {
    std::cerr << "usage: build TYPE1 TYPE2 TYPE3 TYPE4 TYPE5 ROOT FILTER_LEVEL GRAPH SUMMARY LABEL [WORKERS]\n";
    return 2;
  }
  std::array<Type, 5> palette{parse_type(argv[1]), parse_type(argv[2]),
      parse_type(argv[3]), parse_type(argv[4]), parse_type(argv[5])};
  std::array<uint64_t, 5> palette_codes{type_code(palette[0]),
      type_code(palette[1]), type_code(palette[2]), type_code(palette[3]),
      type_code(palette[4])};
  Type root_type = parse_type(argv[6]);
  int filter_level = std::stoi(argv[7]);
  int workers = argc == 12 ? std::max(1, std::stoi(argv[11])) : 1;
  ...
  summary << "Filtered five-type candidate graph\n\n"
          << "label: " << argv[10] << '\n'
          << "palette: " << argv[1] << " + " << argv[2] << " + " << argv[3]
          << " + " << argv[4] << " + " << argv[5] << '\n'
          << "root: " << argv[6] << '\n'
