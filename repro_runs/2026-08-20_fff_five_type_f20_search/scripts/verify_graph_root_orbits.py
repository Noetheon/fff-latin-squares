...
def main() -> None:
    ...
    if not all(dimension_checks.values()):
        raise SystemExit("invalid f20 dimension record")
    ...
    if root != expected_root:
        raise SystemExit("graph does not contain the canonical f20 root")
    ...
    expected_orbit_size_counts = {12: 6, 24: 2, 48: 130}
    if orbit_size_counts != expected_orbit_size_counts:
        raise SystemExit("unexpected f20 orbit-size distribution")
    ...
    args.summary.write_text("\n".join([
        "Five-type f20 graph root-orbit audit", "",
