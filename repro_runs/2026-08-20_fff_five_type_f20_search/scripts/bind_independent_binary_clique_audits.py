...
F20_GRAPH_SHA = "a6a744038dd1cde7372214f9bc0f373aba32af8fdd37b8ba3b70bd1beca31282"
...
def main() -> None:
    ...
    parser.add_argument("--f20-graph", type=Path, required=True)
    parser.add_argument("--f20-result", type=Path, required=True)
    ...
    f20 = json.loads(args.f20_result.read_text())
    ...
    checks = {
        "f20_graph_hash": sha256(args.f20_graph) == F20_GRAPH_SHA,
        "f18_graph_hash": sha256(args.f18_graph) == F18_GRAPH_SHA,
        "f20_result_valid": f20.get("valid") is True and zero_errors(f20),
        "f20_complete_counts": f20.get("initial_tasks") == 6360
        and f20.get("clique_count") == 23040
        and f20.get("pattern_counts") == {"FFT": 11520, "FTF": 11520}
        and f20.get("exact_palette_count") == 19200
        and f20.get("fff_count") == 0,
    ...
    result = {
        "schema_version": "f20-independent-binary-clique-audit-binding-v1",
        "files": {
            "f20_graph": artifact(args.f20_graph),
            "f20_result": artifact(args.f20_result),
        ...
        "commands": {
            ...
            "f20_run": [str(args.binary), str(args.f20_graph),
                "2+2+2+2+2", "4+4+2", "6+2+2", "6+4", "8+2", "8"],
        },
        "checks": checks,
    ...
    args.summary.write_text("\n".join([
        ...
        f"f20 cliques/patterns: {f20.get('clique_count')}/{f20.get('pattern_counts')}",
        f"f20 exact-palette/FFF: {f20.get('exact_palette_count')}/{f20.get('fff_count')}",
