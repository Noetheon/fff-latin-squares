...
F13_GRAPH_SHA = "ee0ab9100c65773e365648db8f902bd757c4af79379c88dbff021eada0955542"
F13_CLIQUE_COUNT = 17280
F13_PATTERN_COUNTS = {"FTT": 9600, "FTF": 3840, "FFT": 3840}
F13_EXACT_PALETTE_COUNT = 13632
...
def main() -> None:
    ...
    parser.add_argument("--f13-graph", type=Path, required=True)
    parser.add_argument("--f13-result", type=Path, required=True)
    ...
    f13 = json.loads(args.f13_result.read_text())
    ...
    checks = {
        "f13_graph_hash": sha256(args.f13_graph) == F13_GRAPH_SHA,
        "f18_graph_hash": sha256(args.f18_graph) == F18_GRAPH_SHA,
        "f13_result_valid": f13.get("valid") is True and zero_errors(f13),
        "f13_complete_counts": f13.get("initial_tasks") == 15036
        and f13.get("clique_count") == F13_CLIQUE_COUNT
        and f13.get("pattern_counts") == F13_PATTERN_COUNTS
        and f13.get("exact_palette_count") == F13_EXACT_PALETTE_COUNT
        and f13.get("fff_count") == 0,
    ...
    result = {
        "schema_version": "f13-independent-binary-clique-audit-binding-v1",
        "files": {
            "f13_graph": artifact(args.f13_graph),
            "f13_result": artifact(args.f13_result),
        ...
        "commands": {
            ...
            "f13_run": [str(args.binary), str(args.f13_graph),
                "10", "4+2+2+2", "4+4+2", "6+4", "8+2", "8"],
        },
        "checks": checks,
    ...
    args.summary.write_text("\n".join([
        ...
        f"f13 cliques/patterns: {f13.get('clique_count')}/{f13.get('pattern_counts')}",
        f"f13 exact-palette/FFF: {f13.get('exact_palette_count')}/{f13.get('fff_count')}",
