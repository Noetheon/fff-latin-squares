...
def main() -> None:
    ...
    if not all(dimension_checks.values()):
        raise SystemExit("invalid f13 dimension record")
    ...
    if root != expected_root:
        raise SystemExit("graph does not contain the canonical f13 root")
    ...
    expected_orbit_size_counts = {12: 3, 24: 1, 48: 312}
    if orbit_size_counts != expected_orbit_size_counts:
        raise SystemExit("unexpected f13 orbit-size distribution")
    ...
    args.summary.write_text("\n".join([
        "Five-type f13 graph root-orbit audit", "",
