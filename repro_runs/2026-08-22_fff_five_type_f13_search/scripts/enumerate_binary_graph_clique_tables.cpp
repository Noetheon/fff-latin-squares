...
struct Enumerator {
  ...
  std::array<std::array<std::vector<uint64_t>, 8>, 8> scratch;

  Enumerator(const Graph &graph_value,
             const std::array<std::vector<uint32_t>, 8> &bucket_values,
             const std::array<std::vector<uint64_t>, 8> &bucket_bit_values,
             const std::vector<uint64_t> &palette_value)
      : graph(graph_value), buckets(bucket_values), bucket_bits(bucket_bit_values),
        palette(palette_value) {
    for (int bucket = 0; bucket < 8; ++bucket) {
      word_begin[bucket] = buckets[bucket].front() / 64;
      uint32_t word_end = buckets[bucket].back() / 64 + 1;
      for (int depth = 0; depth < 8; ++depth) {
        scratch[depth][bucket].resize(word_end - word_begin[bucket]);
      }
    }
  }
  ...
  void dfs(uint8_t unused, int depth) {
    if (!unused) {
      complete();
      return;
    }
    ...
    for (int bucket = 0; bucket < 8; ++bucket) {
      ...
      uint64_t size = bit_count(scratch[depth][bucket]);
      if (!size) return;
      if (size < smallest) {
        smallest = size;
    ...
    const auto &selected = scratch[depth][selected_bucket];
    for (uint32_t local_word = 0; local_word < selected.size(); ++local_word) {
      uint64_t bits = selected[local_word];
      while (bits) {
        uint32_t global_word = word_begin[selected_bucket] + local_word;
        uint32_t vertex = global_word * 64 + std::countr_zero(bits);
        bits &= bits - 1;
        chosen[selected_bucket] = vertex;
        for (int bucket = 0; bucket < 8; ++bucket) {
          if (!(next_unused & (1u << bucket))) continue;
          const auto &current = scratch[depth][bucket];
          auto &next = scratch[depth + 1][bucket];
        ...
        dfs(next_unused, depth + 1);
      }
    }
  }

  void run_root(uint32_t root_vertex) {
    chosen[0] = root_vertex;
    for (int bucket = 1; bucket < 8; ++bucket) {
      auto &initial = scratch[0][bucket];
      uint32_t begin = word_begin[bucket];
      for (uint32_t word = 0; word < initial.size(); ++word) {
        initial[word] = graph.adjacency[root_vertex][begin + word]
            & bucket_bits[bucket][begin + word];
      }
    }
    dfs(0xfe, 0);
  }
...
int main(int argc, char **argv) {
  ...
  auto worker = [&]() {
    ...
    while (true) {
      ...
      enumerator.run_root(buckets[0][task]);
    }
    std::lock_guard<std::mutex> lock(mutex);
    total.cliques += enumerator.totals.cliques;
    total.latin_errors += enumerator.totals.latin_errors;
    total.row_not_f_errors += enumerator.totals.row_not_f_errors;
    total.palette_subset_errors += enumerator.totals.palette_subset_errors;
    total.exact_palette += enumerator.totals.exact_palette;
    total.fff += enumerator.totals.fff;
    for (int i = 0; i < 8; ++i) total.patterns[i] += enumerator.totals.patterns[i];
  };
