...
GRAPH_SHA256 = "08eba5ca2739afa594c620bd6ba620f3b16c96805f9929f969e44e398e97ff12"
...
def main() -> None:
    ...
    structural = load(results / "f07_structural_graph_audit.json")
    semantic = load(results / "f07_complete_graph_audit.json")
    binding = load(results / "f07_graph_audit_binding.json")
    source_audit = load(results / "f07_source_generalization_audit.json")
    orbit = load(results / "f07_graph_root_orbit_audit.json")
    sliced_plan = load(results / "f07_task_plan_sliced.json")
    monolithic_plan = load(results / "f07_task_plan_monolithic.json")
    full_plan = load(results / "f07_task_plan_full_root_bucket.json")
    sliced = load(results / "f07_final_sliced_merge.json")
    monolithic = load(results / "f07_final_monolithic_merge.json")
    full = load(results / "f07_full_root_bucket_merge.json")
    equivalence = load(results / "f07_sliced_monolithic_equivalence.json")
    independent = load(results / "f07_independent_binary_clique_binding.json")
    graph = Path(binding["graph_path"])
    task_file = args.run / "data" / "f07_task_ordinals.txt"

    expected_orbit_sizes = {"12": 6, "24": 2, "48": 170}
    ...
    checks = {
        ...
        and structural.get("vertices") == 66240
        and structural.get("words") == 1035
        and structural.get("actual_size_bytes") == 549129626,
        ...
        and semantic.get("stored_vertices") == 66240
        and semantic.get("independently_enumerated_vertices") == 66240
        and semantic.get("pairs_checked") == 2193835680
        and semantic.get("edges") == 161880000,
        ...
        == sha256(results / "f07_structural_graph_audit.json")
        and binding.get("semantic_audit_sha256")
        == sha256(results / "f07_complete_graph_audit.json"),
        ...
        "orbit_cover_exact": orbit.get("full_initial_task_count") == 8280
        and orbit.get("orbit_count") == 178
        and orbit.get("orbit_size_counts") == expected_orbit_sizes
        and orbit.get("covered_task_count") == 8280
        ...
        and len(task_file.read_text().splitlines()) == 178,
        ...
        "reduced_sliced_complete_negative": complete_negative(
            sliced, 178, 4106815, 1858732
        ),
        "reduced_monolithic_complete_negative": complete_negative(
            monolithic, 178, 4106815, 1858732
        ),
        ...
        and full_plan["tasking"]["total_initial_tasks"] == 8280,
        "full_root_bucket_complete_negative": complete_negative(
            full, 8280, 183635520, 82000920
        ),
        ...
        and independent["files"]["f07_graph"]["sha256"] == GRAPH_SHA256
        and independent["files"]["f07_result"]["sha256"]
        == sha256(results / "f07_binary_clique_table_audit.json")
        and independent["files"]["source"]["sha256"]
        == sha256(args.run / "scripts" / "enumerate_binary_graph_clique_tables.cpp"),
    }
    result = {
        "schema_version": "five-type-f07-run-validation-v1",
        "checks": checks,
        "all_checks_passed": all(checks.values()),
        "result": {
            "case_id": "f07",
            "palette": ["10", "2+2+2+2+2", "4+4+2", "6+2+2", "6+4"],
            "graph_sha256": GRAPH_SHA256,
            "vertices": 66240,
            "edges": 161880000,
            "orbits": 178,
            "full_tasks": 8280,
            "reduced_nodes": 4106815,
            "full_root_nodes": 183635520,
            "fff_found": False,
    ...
    args.summary.write_text("\n".join([
        "Five-type f07 run validation", "",
        f"checks passed: {sum(checks.values())}/{len(checks)}",
        "graph vertices/edges: 66240/161880000",
        "full tasks/orbits: 8280/178",
        "reduced/full-root nodes: 4106815/183635520",
    ...
    if not result["all_checks_passed"]:
        ...
        raise SystemExit(f"f07 validation failed: {failed}")
