...
def main() -> None:
    ...
    if not all(dimension_checks.values()):
        raise SystemExit("invalid f07 dimension record")
    ...
    if root != expected_root:
        raise SystemExit("graph does not contain the canonical f07 root")
    ...
    expected_orbit_size_counts = {12: 6, 24: 2, 48: 170}
    if orbit_size_counts != expected_orbit_size_counts:
        raise SystemExit("unexpected f07 orbit-size distribution")
    ...
    args.summary.write_text("\n".join([
        "Five-type f07 graph root-orbit audit", "",
