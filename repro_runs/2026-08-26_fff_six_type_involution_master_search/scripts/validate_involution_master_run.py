...
GRAPH_SHA256 = "b878a2feddc48629401864c675af6ec6f792290b75df343edd95411fd2326047"
REDUCED_NODES = 5001728282
REDUCED_PRUNES = 3586770865
CONTROL_NODES = 5001728282
CONTROL_PRUNES = 3586770865
...
def main() -> None:
    ...
    if any(value is None for value in frozen):
        raise SystemExit("involution master completion counts are not frozen yet")

    results = args.run / "results"
    inventory_results = args.run.parent / "2026-08-26_fff_six_type_master_inventory" / "results"
    structural = load(results / "involution_graph_structural_audit.json")
    semantic = load(results / "involution_graph_complete_audit.json")
    binding = load(results / "involution_graph_audit_binding.json")
    orbit = load(results / "involution_graph_root_orbit_audit.json")
    reduced_plan = load(results / "involution_task_plan_reduced.json")
    control_plan = load(results / "involution_task_plan_control.json")
    reduced = load(results / "involution_reduced_merge.json")
    control = load(results / "involution_control_merge.json")
    comparison = load(results / "involution_reduced_control_comparison.json")
    validator_audit = load(results / "involution_validator_fail_closed_audit.json")
    ...
    task_file = args.run / "data" / "involution_task_ordinals.txt"

    checks = {
        "master_inventory_bound": inventory.get("all_checks_passed") is True
        and inventory["cases"]["involution_master_all_7_types"]["triple_filtered_candidates"] == 190080
        and inventory["cases"]["involution_master_all_7_types"]["root_task_count"] == 23760
        and inventory["cases"]["involution_master_all_7_types"]["orbit_count"] == 506,
        ...
        and structural.get("vertices") == 190080
        and structural.get("words") == 2970
        and structural.get("actual_size_bytes") == 4518201626,
        "semantic_graph_audit": semantic.get("valid") is True
        and semantic.get("stored_vertices") == 190080
        and semantic.get("independently_enumerated_vertices") == 190080
        and semantic.get("pairs_checked") == 18065108160
        and semantic.get("edges") == 2050329600,
        ...
        and binding.get("structural_audit_sha256") == sha256(results / "involution_graph_structural_audit.json")
        and binding.get("semantic_audit_sha256") == sha256(results / "involution_graph_complete_audit.json"),
        "orbit_cover_exact": orbit.get("valid") is True
        and all(orbit.get("checks", {}).values())
        and orbit.get("graph_sha256") == GRAPH_SHA256
        and orbit.get("full_initial_tasks") == 23760
        and orbit.get("orbit_count") == 506
        and orbit.get("orbit_size_counts") == {"12": 12, "24": 4, "48": 490}
        and orbit.get("covered_task_count") == 23760,
        ...
        and len(task_file.read_text().splitlines()) == 506,
        "plans_bound": reduced_plan["graph"]["sha256"] == control_plan["graph"]["sha256"] == GRAPH_SHA256
        and reduced_plan["tasking"]["total_initial_tasks"] == 506
        and control_plan["tasking"]["total_initial_tasks"] == 506
        ...
        "reduced_complete_negative": complete_negative(reduced, 506, REDUCED_NODES, REDUCED_PRUNES),
        "control_complete_negative": complete_negative(
            control, 506, CONTROL_NODES, CONTROL_PRUNES
        ),
        "reduced_control_exact_match": merge_comparison_valid(
            comparison, 506, REDUCED_NODES, REDUCED_PRUNES
        ),
    ...
    result = {
        "schema_version": "involution-six-seven-type-master-run-validation-v1",
        "checks": checks,
        "all_checks_passed": all(checks.values()),
        "result": {
            "case_id": "involution_master_all_7_types",
            "palette": ["10", "2+2+2+2+2", "4+2+2+2", "4+4+2", "6+2+2", "6+4", "8+2"],
    ...
    args.summary.write_text("\n".join([
        "Involution six-type master run validation", "",
        f"checks passed: {sum(checks.values())}/{len(checks)}",
        "graph vertices/edges: 190080/2050329600",
        "full tasks/orbits: 23760/506",
        "FFF found: false",
        f"all checks passed: {str(result['all_checks_passed']).lower()}",
    ]) + "\n")
    if not result["all_checks_passed"]:
        failed = [name for name, passed in checks.items() if not passed]
        raise SystemExit(f"involution master validation failed: {failed}")
