...
def main() -> None:
    ...
    keys = (
        "completed_tasks", "total_initial_tasks", "task_file_sha256",
        "task_list_sha256", "graph_sha256", "search_binary_sha256",
        "source_bundle_sha256", "search_nodes",
        "crossview_odd_cycle_prunes", "fff_found",
        "negative_search_complete", "complete_disjoint_cover", "status",
    )
    checks = {key: left.get(key) == right.get(key) for key in keys}
    result = {
        "schema_version": "master-search-merge-equivalence-v1",
        "label": args.label,
