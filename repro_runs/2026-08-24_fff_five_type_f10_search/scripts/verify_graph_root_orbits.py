...
EXPECTED_PALETTE = ["10", "2+2+2+2+2", "6+2+2", "6+4", "8+2"]
...
def main() -> None:
    ...
    dimension_checks = {
        ...
        "case": dimension.get("case_id") == "f10",
        "palette": dimension.get("palette") == EXPECTED_PALETTE,
        "root_type": dimension.get("root_type") == "2+2+2+2+2",
        "expected_counts": dimension.get("level1_candidates") == 161280
        and dimension.get("full_initial_tasks") == 20160
        and dimension.get("orbit_count") == 420,
    }
    if not all(dimension_checks.values()):
        raise SystemExit("invalid f10 dimension record")
    ...
    if root != EXPECTED_ROOT:
        raise SystemExit("graph does not contain the canonical f10 root")
    ...
    if orbit_size_counts != EXPECTED_ORBIT_SIZES:
        raise SystemExit("unexpected f10 orbit-size distribution")
    ...
    args.summary.write_text("\n".join([
        "Five-type f10 graph root-orbit audit", "",
