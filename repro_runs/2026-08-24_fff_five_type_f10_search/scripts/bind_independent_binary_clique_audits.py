...
F10_GRAPH_SHA = "3be70282184b508938c8fea80a0767be6fe58715ca35a50d0dcc00efaa635f97"
...
def main() -> None:
    ...
    parser.add_argument("--f10-graph", type=Path, required=True)
    parser.add_argument("--f10-result", type=Path, required=True)
    ...
    f10 = json.loads(args.f10_result.read_text())
    f18 = json.loads(args.f18_result.read_text())
    f18_reference = json.loads(args.f18_reference_audit.read_text())
    f10_patterns = f10.get("pattern_counts", {})
    checks = {
        "f10_graph_hash": sha256(args.f10_graph) == F10_GRAPH_SHA,
        "f18_graph_hash": sha256(args.f18_graph) == F18_GRAPH_SHA,
        "f10_result_valid": f10.get("valid") is True and zero_errors(f10),
        "f10_complete_task_cover": f10.get("initial_tasks") == 20160,
        "f10_pattern_partition": sum(f10_patterns.values()) == f10.get("clique_count")
        and f10_patterns.get("FFF", 0) == f10.get("fff_count"),
        "f10_no_fff": f10.get("fff_count") == 0,
        "f10_exact_palette_count_bounded": 0 <= f10.get("exact_palette_count", -1)
        <= f10.get("clique_count", -2),
    ...
    result = {
        "schema_version": "f10-independent-binary-clique-audit-binding-v1",
        "files": {
            "f10_graph": artifact(args.f10_graph),
            "f10_result": artifact(args.f10_result),
        ...
        "commands": {
            ...
            "f10_run": [str(args.binary), str(args.f10_graph),
                "10", "2+2+2+2+2", "6+2+2", "6+4", "8+2", "8"],
        },
        "checks": checks,
    ...
    args.summary.write_text("\n".join([
        ...
        f"f10 cliques/patterns: {f10.get('clique_count')}/{f10_patterns}",
        f"f10 exact-palette/FFF: {f10.get('exact_palette_count')}/{f10.get('fff_count')}",
