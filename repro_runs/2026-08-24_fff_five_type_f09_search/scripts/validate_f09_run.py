...
GRAPH_SHA256 = "8ebf555e63f51e0fad9d24f71fc12e47ac057ff55ed22b640f4d7c954f0d878e"
FULL_ROOT_NODES = 32020758728
FULL_ROOT_PRUNES = 22043929220
TRANSVERSAL_CLIQUE_COUNT = 23040
EXACT_PALETTE_CLIQUE_COUNT = 21120
...
def main() -> None:
    ...
    structural = load(results / "f09_structural_graph_audit.json")
    semantic = load(results / "f09_complete_graph_audit.json")
    binding = load(results / "f09_graph_audit_binding.json")
    source_audit = load(results / "f09_source_generalization_audit.json")
    orbit = load(results / "f09_graph_root_orbit_audit.json")
    sliced_plan = load(results / "f09_task_plan_sliced.json")
    monolithic_plan = load(results / "f09_task_plan_monolithic.json")
    full_plan = load(results / "f09_task_plan_full_root_bucket.json")
    sliced = load(results / "f09_final_sliced_merge.json")
    monolithic = load(results / "f09_final_monolithic_merge.json")
    full = load(results / "f09_full_root_bucket_merge.json")
    equivalence = load(results / "f09_sliced_monolithic_equivalence.json")
    independent = load(results / "f09_independent_binary_clique_binding.json")
    graph = Path(binding["graph_path"])
    task_file = args.run / "data" / "f09_task_ordinals.txt"
    ...
    checks = {
        ...
        and structural.get("vertices") == 150720
        and structural.get("words") == 2355
        and structural.get("actual_size_bytes") == 2841072026,
        ...
        and semantic.get("stored_vertices") == 150720
        and semantic.get("independently_enumerated_vertices") == 150720
        and semantic.get("pairs_checked") == 11358183840
        and semantic.get("edges") == 1079770560,
        ...
        == sha256(results / "f09_structural_graph_audit.json")
        and binding.get("semantic_audit_sha256")
        == sha256(results / "f09_complete_graph_audit.json"),
        ...
        "orbit_cover_exact": orbit.get("full_initial_task_count") == 18840
        and orbit.get("orbit_count") == 398
        and orbit.get("orbit_size_counts") == expected_orbit_sizes
        and orbit.get("covered_task_count") == 18840
        ...
        and len(task_file.read_text().splitlines()) == 398,
        ...
        "reduced_sliced_complete_negative": complete_negative(
            sliced, 398, 693853301, 478901095
        ),
        "reduced_monolithic_complete_negative": complete_negative(
            monolithic, 398, 693853301, 478901095
        ),
        ...
        and full_plan["tasking"]["total_initial_tasks"] == 18840,
        "full_root_bucket_complete_negative": complete_negative(
            full, 18840, FULL_ROOT_NODES, FULL_ROOT_PRUNES
        ),
        ...
        and independent["files"]["f09_graph"]["sha256"] == GRAPH_SHA256
        and independent["files"]["f09_result"]["sha256"]
        == sha256(results / "f09_binary_clique_table_audit.json")
        and independent["files"]["source"]["sha256"]
        == sha256(args.run / "scripts" / "enumerate_binary_graph_clique_tables.cpp"),
    }
    result = {
        "schema_version": "five-type-f09-run-validation-v1",
        "checks": checks,
        "all_checks_passed": all(checks.values()),
        "result": {
            "case_id": "f09",
            "palette": ["10", "2+2+2+2+2", "4+4+2", "6+4", "8+2"],
            "graph_sha256": GRAPH_SHA256,
            "vertices": 150720,
            "edges": 1079770560,
            "orbits": 398,
            "full_tasks": 18840,
            "reduced_nodes": 693853301,
    ...
    args.summary.write_text("\n".join([
        "Five-type f09 run validation", "",
        f"checks passed: {sum(checks.values())}/{len(checks)}",
        "graph vertices/edges: 150720/1079770560",
        "full tasks/orbits: 18840/398",
    ...
    if not result["all_checks_passed"]:
        ...
        raise SystemExit(f"f09 validation failed: {failed}")
