...
def main() -> None:
    ...
    if not all(dimension_checks.values()):
        raise SystemExit("invalid f03 dimension record")
    ...
    if root != expected_root:
        raise SystemExit("graph does not contain the canonical f03 root")
    ...
    expected_orbit_size_counts = {48: 227}
    if orbit_size_counts != expected_orbit_size_counts:
        raise SystemExit("unexpected f03 orbit-size distribution")
    ...
    args.summary.write_text("\n".join([
        "Five-type f03 graph root-orbit audit", "",
