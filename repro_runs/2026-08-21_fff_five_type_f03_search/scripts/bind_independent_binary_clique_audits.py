...
F03_GRAPH_SHA = "0dd0d6924c2544f8d3c69d744e7adb0a0d871d2833c5edd969a2668a7ae803fe"
F03_CLIQUE_COUNT = 768
F03_PATTERN_COUNTS = {"FFT": 384, "FTF": 384}
F03_EXACT_PALETTE_COUNT = 0
...
def main() -> None:
    ...
    parser.add_argument("--f03-graph", type=Path, required=True)
    parser.add_argument("--f03-result", type=Path, required=True)
    ...
    f03 = json.loads(args.f03_result.read_text())
    ...
    checks = {
        "f03_graph_hash": sha256(args.f03_graph) == F03_GRAPH_SHA,
        "f18_graph_hash": sha256(args.f18_graph) == F18_GRAPH_SHA,
        "f03_result_valid": f03.get("valid") is True and zero_errors(f03),
        "f03_complete_counts": f03.get("initial_tasks") == 10896
        and f03.get("clique_count") == F03_CLIQUE_COUNT
        and f03.get("pattern_counts") == F03_PATTERN_COUNTS
        and f03.get("exact_palette_count") == F03_EXACT_PALETTE_COUNT
        and f03.get("fff_count") == 0,
    ...
    result = {
        "schema_version": "f03-independent-binary-clique-audit-binding-v1",
        "files": {
            "f03_graph": artifact(args.f03_graph),
            "f03_result": artifact(args.f03_result),
        ...
        "commands": {
            ...
            "f03_run": [str(args.binary), str(args.f03_graph),
                "10", "2+2+2+2+2", "4+2+2+2", "4+4+2", "8+2", "8"],
        },
        "checks": checks,
    ...
    args.summary.write_text("\n".join([
        ...
        f"f03 cliques/patterns: {f03.get('clique_count')}/{f03.get('pattern_counts')}",
        f"f03 exact-palette/FFF: {f03.get('exact_palette_count')}/{f03.get('fff_count')}",
